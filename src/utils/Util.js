export function getAirQualityInterpretation(so2 = 0, no2 = 0, co = 0, co2 = 0) {
    ...
    if (so2Interpretation.colorCode === "Red" || no2Interpretation.colorCode === "Red" || co2Interpretation.colorCode === "Red") {
        areaColor = "#FF0000"; // High risk - Red
        insights = {
            riskLevel: "High Risk (Unhealthy Air Quality)",
            message: "Air quality is unhealthy today. Everyone, especially sensitive groups, should limit outdoor activities.",
            educationalInsight: "Prolonged exposure to high pollution levels can exacerbate respiratory issues, leading to increased symptoms or flare-ups. Protect yourself by reducing outdoor exposure.",
            precaution: "Stay indoors with windows closed as much as possible. Use an air purifier if available and avoid outdoor exercise. Wear an N95 mask if you need to go outside."
        };
    } else if (so2Interpretation.colorCode === "Yellow" || no2Interpretation.colorCode === "Yellow" || co2Interpretation.colorCode === "Yellow") {
        areaColor = "#FFC000"; // Moderate risk - Yellow
        insights = {
            riskLevel: "Moderate Risk (Moderate Air Quality)",
            message: "Air quality is moderate today. Sensitive groups, such as those with asthma or COPD, should reduce prolonged outdoor exertion.",
            educationalInsight: "People with respiratory conditions might experience mild symptoms such as coughing or shortness of breath. Keep track of your symptoms and avoid triggers.",
            precaution: "If you're sensitive to air pollution, consider staying indoors during peak pollution hours or wearing a mask when outside."
        };
    } else if (so2Interpretation.colorCode === "Green" || no2Interpretation.colorCode === "Green" || co2Interpretation.colorCode === "Green") {
        areaColor = "#98FB98"; // Low risk - Green
        insights = {
            riskLevel: "Low Risk (Good Air Quality)",
            message: "Air quality is good today! You can enjoy outdoor activities without any concerns.",
            educationalInsight: "Breathing clean air supports overall health and well-being. Keep an eye on air quality to continue protecting your lungs.",
            precaution: "No specific precautions are needed, but maintaining a healthy lifestyle with regular exercise and a balanced diet is always beneficial."
        };
    } else {
        areaColor = "#ff453a80"; // Default
        insights = {
            riskLevel: "Critical Risk (Hazardous Air Quality)",
            message: "Air quality is hazardous today. Everyone should avoid outdoor activities.",
            educationalInsight: "At hazardous levels, air pollution can cause serious health effects, including respiratory and cardiovascular issues. It's essential to minimize exposure.",
            precaution: "Stay indoors with air purification if possible. Seal windows and doors to prevent outdoor air from entering. Avoid any physical exertion outdoors. Seek medical advice if you experience any symptoms."
        };
    }
...
export const getAQICategory = (pm25Value) => {
    if (pm25Value >= 0 && pm25Value <= 30) {
        return {
            ...
            message: "Great news! The air quality is expected to be good today. Feel free to plan outdoor activities.",
            insight: "Encourage maintaining regular outdoor routines for a healthy lifestyle.",
        };
    } else if (pm25Value >= 31 && pm25Value <= 60) {
        return {
            quality: "AQI Category 2 (Satisfactory)",
            message: "The air quality is expected to be good today. Feel free to plan outdoor activities.",
            insight: "Encourage maintaining regular outdoor routines for a healthy lifestyle.",
        };
    } else if (pm25Value >= 61 && pm25Value <= 90) {
        return {
            quality: "AQI Category 3 (Moderate)",
            message: "The air quality is expected to be moderate. Consider limiting outdoor activities if you have respiratory conditions.",
        ...
        return {
            ...
            message: "Air quality is predicted to be unhealthy. It’s a good idea to limit outdoor activities and stay indoors",
            insight: "Recommend precautions like avoiding outdoor exercise and using masks.",
        };
    } else if (pm25Value >= 121 && pm25Value <= 250) {
        return {
            quality: "AQI Category 5 (Very Poor)",
            message: "Air quality is expected to reach hazardous levels. Stay indoors and minimize exposure.",
            insight: "Strongly advise avoiding outdoor activities and focus on keeping indoor air clean.",
        };
        ...
        leg.steps.forEach(step => {
            ...
            if (matches) {
                matches.forEach(match => {
                    ...
                    if (!['Head', 'Turn', 'Continue', 'Pass', 'Merge', 'At', 'Slight', 'Destination', 'Restricted', 'Keep', 'Exit'].includes(match)) {
...
export const formatRoutes = (routes) => {
    return routes.map(route => {
        const cities = route.legs[0].steps.map(step => step.html_instructions.replace(/<[^>]+>/g, ''));
