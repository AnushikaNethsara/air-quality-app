...
const AirPollutionChartScreen = ({ route, navigation }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                setIsLoading(true)
                const response = await axios.get(constants.backend_url + "/levels/a-city-history/" + city);
                console.log("\n\n\nresponse: screen***");
                const sortedData = await response?.data?.sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp));
